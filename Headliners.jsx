...
const ALL_COUNCIL_OBJECTIVES = [
  ...
  { id: "party_animals", name: "Party Animals", flavour: "Some campers love being close to the action.", req: "Campsites between stages", benefit: "+3 tickets/campsite" },
  ...
  { id: "shepherds", name: "Shepherds", flavour: "Guiding people between stages.", req: "Security on tiles between two stages", benefit: "+2 tickets/security" },
  ...
  // Catering-focused
  { id: "stage_sandwich", name: "Stage Sandwich", flavour: "Food between the action.", req: "Catering vans between two stages (within 3 tiles of each)", benefit: "+3 tickets/catering" },
...
function evalCouncilObjective(obj, pd, isTrending) {
  ...
  switch (obj.id) {
    ...
    case "no_wristband": {
      count = ofType("security").filter(s => ofType("campsite").some(c => stages.some(st => { const dc = hexDistance(s.col,s.row,c.col,c.row); const ds = hexDistance(s.col,s.row,st.col,st.row); const cs = hexDistance(c.col,c.row,st.col,st.row); return dc < cs && ds < cs && dc + ds <= cs + 1; }))).length;
...
export default function Headliners() {
  ...
  const resolveYearEndEffect = (result) => {
    ...
    if (effect.type === "autoVP" || effect.type === "fameVP") {
      ...
      // Result comes from dice roll callback
      if (result?.vp) {
  ...
  const advanceYearEndEffect = () => {
    ...
    if (yearEndEffectIdx < effects.length - 1) {
      ...
      if (nextPlayer < players.length) {
        ...
        setTimeout(() => { recalcTickets(); beginEventPhase(); }, 100);
      }
    }
  };
  ...
  const updateNotesBtn = <button onClick={() => setShowUpdateNotes(true)} style={{ position: "fixed", top: 16, right: showLog ? 470 : showDiscard ? 230 : 160, zIndex: 999, padding: "8px 16px", borderRadius: 10, border: "1px solid #22c55e", background: "rgba(34,197,94,0.2)", color: "#4ade80", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>📋 Updates</button>;
  ...
  if (phase === "game") {
    ...
    return (<div style={CS}>{logBtn}{discardBtn}{updateNotesBtn}{updateNotesModal}
